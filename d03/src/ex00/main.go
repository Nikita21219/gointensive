...
func mappingIndex(indexName string, es *elastic.Client) error {
	...
	schema := readDB("src/ex00/schema.json", new(JsonReader))
...
func pushIntoES(indexName string, line []string, es *elastic.Client, idx int) error {
	id := strconv.Itoa(idx)
...
func fillDataFromCSV(indexName, pathToCSV string, es *elastic.Client) error {
	...
	for idx, line := range lines[1:] {
		...
		go func() {
			...
			if err != nil {
				fmt.Println("Error push data into elastic:", err)
			}
		}()
	}
	wg.Wait()
...
func main() {
	...
	err = fillDataFromCSV(indexName, "materials/data.csv", es)
	if err != nil {
		log.Fatalf("Error mapping the index: %s\n", err)
	}
}
